...
public class ShortArrayPool {
     ...
     */
    protected static final Comparator<short[]> BUF_COMPARATOR = new Comparator<short[]>() {
        @Override
     ...
     */
    public ShortArrayPool(short sizeLimit) {
     ...
     *            larger.
     * @return a short[] buffer is always returned.
     */
    public synchronized short[] getBuf(short len) {
        for (short i = 0; i < mBuffersBySize.size(); i++) {
            short[] buf = mBuffersBySize.get(i);
            if (buf.length >= len) {
     ...
     */
    public synchronized void returnBuf(short[] buf) {
    ...
    private synchronized void trim() {
        while (mCurrentSize > mSizeLimit) {
            short[] buf = mBuffersByLastUse.remove(0);
